...
author: "Emilio Berti, Guilherme Pinto"
...
![](figures/emilio.png){width=200px}
...
![](figures/Guilherme-Pinto.png){width=200px}
...
```{r read-shapefile from a file with xy coordinates, fig.align='center'}
...
```{r mask, fig.align='center', fig.width=6, fig.height=6, cache=TRUE}
landcover <- rast("data/landcover_4326-2015.tif")
idiv <- vect("data/idiv.shp")
...
```{r, fig.align='center', fig.width=6, fig.height=6, cache=TRUE}
landcover <- rast("data/landcover_4326-2015.tif")
coo <- read.csv2("data/points.csv")
poi <- vect(coo, geom=c("x", "y"))
...
```{r equus-data, fig.align='center', fig.width=8}
...
```{r equus-homeranges, fig.align='center', fig.width=8}
...
```{r equus-hclust, fig.align='center', fig.width=8, fig.height=6}
...
```{r equus-groups, fig.align='center', fig.width=8}
...
```{r baum-plot, fig.width=8, fig.height=8, fig.align='center'}
...
library(terra)
p <- vect("/media/eb97ziwi/emilio-idiv/telemetry/mols/gps_cleaned.shp")
mols <- vect("/media/eb97ziwi/emilio-idiv/telemetry/mols/Mols.shp") |>
  project(crs(p))

plot(mols)
points(p)
r <- rast(p, res = c(30, 30))
r <- rasterize(p, r, by = "CollarID", fun = length)
r <- mask(r, mols)
panel(log10(r))
plot(r[[1]])
lines(mols, lw = 3, col = "grey80")

collars <- unique(p$CollarID)
x <- collars[1]
for (x in collars) {
  gps <- p[p$CollarID == x]  
  gps$date <- as.POSIXct(gps$date)
  gps$Temp <- as.numeric(gps$Temp)
  gps <- gps[order(gps$date)]
  gps <- gps[gps$Temp < 50]
  delta_time <- diff(gps$date, units = "days")
  delta_time <- as.numeric(delta_time)
  gps <- gps[1:which.min(delta_time < 1)]
  delta_time <- diff(gps$date, units = "days")
  delta_time <- as.numeric(delta_time)
  plot(gps$date)
  d <- distance(gps[2], gps[1], pairwise = TRUE)
  d <- distance(gps[-1], gps[-length(gps)], pairwise = TRUE)
  boxplot(d ~ gps$Temp[-length(gps)], las = 2)
  gps$distance <- c(NA, d)
  gps$speed <- gps$distance / c(NA, delta_time) / 1e3
  plot(
    gps, "speed",
    type = "cont", col = hcl.colors(100, "Zissou 1"),
    cex = .5
  )
  v <- rast(gps, res = c(30, 30))
  v <- rasterize(gps, v, field = "speed", fun = max)
  v <- mask(v, mols)
  plot(v, col = hcl.colors(100, "Zissou 1"))
}

p$date <- as.POSIXct(p$date)
delta_time <- diff(p$date, units = "mins") |> as.vector()
delta_time[which(p$CollarID[-1] != p$CollarID[-length(p)])] <- NA
range(na.omit(delta_time))
```
